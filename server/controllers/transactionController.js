...
// const getTransactions = async (req, res) => {
//   try {
//     const { month, search = "", page = 1, perPage = 10 } = req.query; // Get the month, search, page, and perPage from query parameters
//     console.log("Received month:", month); // Log the received month for debugging
...
//     console.log(
//       `Filtering transactions for month: ${month} (Month number: ${monthNumber})`
//     );
...
//       ? {
//           $or: [
//             { productTitle: { $regex: search, $options: "i" } }, // Case-insensitive search on title
...
//     // Combine month and search filters
//     const filter = { ...monthFilter, ...searchFilter };
...
//     // Fetch total transactions count with the applied filter
//     const totalTransactions = await Transaction.countDocuments(filter);

//     // Fetch paginated transactions with the applied filter
//     const transactions = await Transaction.find(filter)
//       .skip(skipItems)
//       .limit(itemsPerPage);

//     console.log(
//       `Found ${transactions.length} transactions for month: ${month} with search: "${search}"`
//     );
